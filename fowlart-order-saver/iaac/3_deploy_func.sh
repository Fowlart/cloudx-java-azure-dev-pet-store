...
url=$(az functionapp show --name fowlart-order-saver --resource-group fowlartordersaver-rg --output tsv --query "hostNames[0]")
...
az webapp config appsettings set --name $ORDER_SERVICE_APP_NAME --resource-group $RG_NAME --settings ORDER_SAVER_URL=$url
